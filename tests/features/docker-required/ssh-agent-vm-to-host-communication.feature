...
Feature: VM-to-Host Communication
  ...
  Scenario: Executing commands on host from VM
    ...
    And I run "to-host docker ps" for VM-to-Host
  ...
  Scenario: Viewing host logs from VM
    ...
    And I run "to-host tail -f /var/log/app.log" for VM-to-Host
  ...
  Scenario: Listing host directories from VM
    ...
    When I SSH into the Python VM for VM-to-Host
    And I run "to-host ls ~" for VM-to-Host
  ...
  Scenario: Checking host resource usage from VM
    ...
    And I run "to-host docker stats" for VM-to-Host
  ...
  Scenario: Managing host containers from VM
    ...
    When I SSH into the Python VM for VM-to-Host
    And I run "to-host docker restart postgres" for VM-to-Host
  ...
  Scenario: Accessing host files from VM
    ...
    When I SSH into the Python VM for VM-to-Host
    And I run "to-host cat ~/dev/config.yaml" for VM-to-Host
  ...
  Scenario: Triggering host builds from VM
    ...
    And I run "to-host cd ~/dev/project && make build" for VM-to-Host
  ...
  Scenario: Coordinating multi-VM operations from host
    ...
    When I SSH into the Go VM for VM-to-Host
    And I run "to-host docker ps --filter 'name=vde-python'" for VM-to-Host
  ...
  Scenario: Host backup operations from VM
    ...
    When I SSH into the Python VM for VM-to-Host
    And I run "to-host ~/dev/bin/backup.sh" for VM-to-Host
  ...
  Scenario: Debugging host issues from VM
    ...
    When I SSH into the Go VM for VM-to-Host
    And I run "to-host systemctl status docker" for VM-to-Host
  ...
  Scenario: Host network operations from VM
    ...
    When I SSH into the Rust VM for VM-to-Host
    And I run "to-host ping -c 3 github.com" for VM-to-Host
  ...
  Scenario: Executing custom host scripts from VM
    ...
    When I SSH into the Python VM for VM-to-Host
    And I run "to-host ~/dev/bin/cleanup.sh" for VM-to-Host
