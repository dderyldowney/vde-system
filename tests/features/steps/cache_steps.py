...
def step_concurrent_read(context):
    ...
    for _ in range(3):
        p = subprocess.Popen(
            ["./bin/vde", "list"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=str(VDE_ROOT)
        )
        processes.append(p)
...
def step_not_reparsed(context):
    """Verify config was not reparsed - cache mtime unchanged."""
    if hasattr(context, "cache_mtime_before"):
        current_mtime = CACHE_FILE.stat().st_mtime
        assert current_mtime == context.cache_mtime_before, (
            "Cache was reparsed when it shouldn't have been"
        )
    else:
        # Fallback: verify cache file exists (implies it was used)
...
def step_reparsed(context):
    """Verify config was reparsed - cache updated."""
    assert CACHE_FILE.exists(), "Cache file should exist after reparse"
    # Verify cache is newer than when we recorded
    if hasattr(context, "cache_mtime_before"):
        current_mtime = CACHE_FILE.stat().st_mtime
        assert current_mtime >= context.cache_mtime_before, "Cache should be updated"
...
def step_cache_format_valid(context):
    ...
    for line in lines:
        ...
        is_valid = (
            ...
            or "=(" in line
            or "export " in line
            or (re.match(r"^[A-Z0-9_]+=[0-9]+$", line))
            or (re.match(r"^[A-Z0-9_]+=", line) and "(" not in line and "[" not in line)
        )
...
def step_ports_available_fast(context):
    """Verify port registry contains data from vm-types cache."""
    registry_path = VDE_ROOT / ".cache" / "port-registry"
...
def step_flag_reset(context):
    """Verify internal flag reset by checking fresh load behavior."""
    # After invalidate, cache file should be gone
    # A new load should recreate it successfully
    result = run_vde_command("list", context=context)
    assert result.returncode == 0, "Should load types successfully after reset"
...
def step_not_initial_load(context):
    """Verify lazy loading behavior - types loaded on demand."""
    # Cache file existing proves lazy loading worked
    assert CACHE_FILE.exists(), "Cache should exist after lazy load"
    # Verify we can read VM types (proves they were loaded)
    result = run_vde_command("list --name-only", context=context)
    assert result.returncode == 0 and len(result.stdout.strip().split("\n")) > 0, (
        "VM types should be loaded"
    )
...
def step_cache_reflects_updates(context):
    """Verify cache update by checking registry content."""
    registry_path = VDE_ROOT / ".cache" / "port-registry"
    # After update, registry should have port files
    assert registry_path.exists(), "Port registry should exist"
    port_files = list(registry_path.glob("*.port"))
    assert len(port_files) > 0, "Registry should contain port allocations"
...
def step_registry_rebuilt(context):
    """Verify registry rebuilt from vm-types source of truth."""
    from config import VDE_CACHE_DIR

    registry_path = VDE_CACHE_DIR / "port-registry"

    # verify_port_registry reads from vm-types.json (source of truth)
    # VM names in vm-types.json include 'vde-' prefix
    port_file = registry_path / "vde-python.port"

    if not port_file.exists():
        if registry_path.exists():
            print(f"DEBUG: Registry exists. Found: {list(registry_path.glob('*.port'))}")
        else:
            print(f"DEBUG: Registry directory {registry_path} is missing")

    assert port_file.exists(), f"Port file {port_file} was not rebuilt from vm-types source"


@then("all allocated ports should be discovered")
def step_ports_discovered(context):
    """Verify all ports discovered in registry from vm-types source."""
    from config import VDE_CACHE_DIR

    registry_path = VDE_CACHE_DIR / "port-registry"

    # verify_port_registry reads from vm-types.json which has all VMs
    # Check that multiple VM ports are present
    port_files = list(registry_path.glob("*.port"))
    assert len(port_files) >= 1, f"Expected at least 1 port file, found {len(port_files)}"

    # Verify ports are in valid ranges
    for pf in port_files:
        port_str = pf.read_text().strip()
        assert port_str.isdigit(), f"Invalid port value: {port_str}"
        port = int(port_str)
        assert (2200 <= port <= 2299) or (2400 <= port <= 2499), f"Port {port} out of valid range"
