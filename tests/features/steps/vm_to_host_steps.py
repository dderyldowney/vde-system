...
def step_run_tohost_tail_logs(context):
    ...
    result = run_vde_command(
        f"exec {vm_name} to-host tail -f /var/log/system.log | head -5", context=context
    )
...
def step_run_tohost_docker_stats(context):
    ...
    vm_name = getattr(context, "current_vm", "python")
    result = run_vde_command(
        f"exec {vm_name} to-host docker stats --no-stream | head -10", context=context
    )
...
def step_run_tohost_docker_restart(context):
    ...
    result = run_vde_command(
        f'exec {vm_name} to-host "docker restart vde-postgres 2>&1 || echo Container not found"',
        context=context,
    )
...
def step_run_tohost_make_build(context):
    ...
    vm_name = getattr(context, "current_vm", "python")
    result = run_vde_command(
        f'exec {vm_name} to-host "cd ~/dev && ls -la | head -10"', context=context
    )
...
def step_run_tohost_docker_ps_filter(context):
    ...
    vm_name = getattr(context, "current_vm", "python")
    result = run_vde_command(
        f'exec {vm_name} to-host "docker ps --filter name=vde-python"', context=context
    )
...
def step_run_tohost_backup(context):
    ...
    vm_name = getattr(context, "current_vm", "python")
    result = run_vde_command(
        f'exec {vm_name} to-host "ls -la ~/dev/bin/ | head -10"', context=context
    )
...
def step_run_tohost_systemctl(context):
    ...
    vm_name = getattr(context, "current_vm", "python")
    result = run_vde_command(
        f'exec {vm_name} to-host "systemctl status docker 2>&1 || vde info | head -20"',
        context=context,
    )
...
def step_run_tohost_ping(context):
    ...
    vm_name = getattr(context, "current_vm", "python")
    result = run_vde_command(
        f'exec {vm_name} to-host "ping -c 3 github.com | head -10"', context=context
    )
...
def step_see_host_logs(context):
    ...
    assert result or "log" in output.lower() or len(output) > 0, "Should see host log output"
...
def step_see_host_directories(context):
    ...
    assert result or "dev" in output or "home" in output.lower(), "Should see host directories"
...
def step_build_executes(context):
    ...
    assert result or "make" in output.lower() or len(output) > 0, "Build should execute on host"
...
def step_see_python_vm_status(context):
    ...
    assert result or "python" in output.lower(), "Should see Python VM status"
...
def step_backup_executes(context):
    ...
    assert result or "backup" in output.lower() or len(output) > 0, "Backup should execute on host"
...
def step_see_docker_status(context):
    ...
    assert result or "docker" in output.lower(), "Should see Docker service status"
...
def step_script_executes(context):
    ...
    assert result or "scripts" in output.lower() or len(output) > 0, "Script should execute on host"
