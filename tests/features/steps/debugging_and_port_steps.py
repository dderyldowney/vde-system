...
def step_volume_mounts(context):
    ...
    output = getattr(context, "vde_command_output", "")
    assert any(
        x in output.lower() for x in ["volume", "mount", "bind", "/home/devuser/workspace"]
...
def step_error_diagnosis(context):
    ...
    output = getattr(context, "vde_command_output", "") or getattr(context, "last_output", "")
    assert any(
        x in output.lower() for x in ["port", "conflict", "docker", "config", "error", "failed"]
...
def step_identify_db_issue(context):
    ...
    if exit_code == 0:
        assert "accepting" in output.lower() or exit_code == 0
    else:
        assert len(output) > 0 or exit_code != 0, f"Should have diagnostic info: {output}"
...
def step_create_new_lang_vm(context):
    ...
    result = run_vde_command(
        f"add {vm_name} --type lang --display Testport{timestamp} --install 'apt-get install -y curl'",
        context=context,
    )
    assert result.returncode == 0, (
        f"Failed to add VM type {vm_name}: {result.stdout}\n{result.stderr}"
    )
    ...
    assert res_create.returncode == 0, (
        f"Failed to create VM after type addition: {res_create.stdout}"
    )
