...
def _add_vm_type_temporarily(context, vm_name, vm_type, install_cmd, display=None, port=None, aliases=None):
...
def _remove_vm_type(vm_name):
    ...
    conf_file.write_text("\n".join(
        l for l in lines if not (search in l and not l.strip().startswith("#"))
    ) + "\n")
...
def step_no_vm_config(context, vm_name):
    """Ensure no compose config exists for vm_name."""
    conf_dir = _vm_conf_dir(vm_name)
    if conf_dir.exists():
...
def step_run_command(context, command):
    ...
    for token in command.replace(" && ", " ").split():
        if token not in (
            "create-virtual-for", "start-virtual", "shutdown-virtual",
            "remove-virtual", "list-vms", "add-vm-type",
            "--lang", "--svc", "--rebuild", "--no-cache", "--type", "--display",
            "lang", "svc", "all",
...
def step_vm_config_still_exists(context):
    ...
    assert compose_file_exists(vm_name), \
        f"Compose file missing for {vm_name}"
...
def step_each_vm_unique_ssh_port(context):
    ...
    assert len(ports_clean) == len(set(ports_clean)), \
        f"Duplicate SSH ports found: {ports_clean}"
...
def step_command_fails_with_error(context, error_text):
    ...
    assert exit_code != 0, \
        f"Expected non-zero exit code, got {exit_code}. Output: {output}"
    assert error_text.lower() in output.lower(), \
        f"Expected '{error_text}' in output:\n{output}"
...
def step_all_lang_vms_listed(context):
    ...
    for vm in ("python", "go", "rust"):
        assert vm in output.lower(), \
            f"Expected '{vm}' in list-vms output:\n{output}"
...
def step_vm_marked_not_created(context):
    vm_name = getattr(context, 'vm_name', 'python')
    assert not compose_file_exists(vm_name), \
        f"Compose file still exists for {vm_name}"
    ...
    for line in content.splitlines():
        if search in line and not line.strip().startswith("#"):
            assert display_name in line, \
                f"Display name '{display_name}' not in line: {line}"
            return
    raise AssertionError(f"VM type '{vm_name}' not found in {conf_file}")
    ...
    for line in content.splitlines():
        if search in line and not line.strip().startswith("#"):
            assert aliases in line, \
                f"Aliases '{aliases}' not found in line: {line}"
            return
    raise AssertionError(f"VM type '{vm_name}' not found in {conf_file}")
...
def step_vm_config_generated(context):
    ...
    assert compose_file_exists(vm_name), \
        f"Compose config not generated for {vm_name}"
...
def step_rust_vm_stops(context):
    assert not container_is_running("rust"), "Rust VM still running"
