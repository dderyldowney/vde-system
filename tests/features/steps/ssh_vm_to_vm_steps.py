...
def step_can_run_psql(context):
    ...
    assert getattr(context, 'psql_success', False)
...
def step_sees_postgres_databases(context):
    ...
    assert 'postgres' in getattr(context, 'psql_output', '') or getattr(context, 'psql_success', False)


@then('I should see "{expected}"')
def step_should_see_expected(context, expected):
    """Verify expected output is present."""
    output = str(getattr(context, 'ssh_output', '')) + \
