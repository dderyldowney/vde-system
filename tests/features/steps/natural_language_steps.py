...
def step_understand_start_go(context):
    ...
    output = getattr(context, "last_output", "") + getattr(context, "last_error", "")
    exit_code = getattr(context, "last_exit_code", -1)
    assert exit_code == 0 or "go" in output.lower(), f"Should understand start Go intent: {output}"
...
def step_understand_create_vms(context):
    ...
    output = getattr(context, "last_output", "") + getattr(context, "last_error", "")
    exit_code = getattr(context, "last_exit_code", -1)
    assert exit_code in [0, 6], f"Should understand create VMs intent: {output}"
...
@then("the JavaScript VM from my command should be created")
...
# ========== GIVEN STEPS (Narrative Markers for User Guide) ==========
# These Given steps set narrative context for documentation purposes.
# They intentionally perform no setup - the actual state is established by When/Then steps.


@given("I want to perform common actions")
def step_want_common_actions(context):
    """NARRATIVE: User wants to perform common VM actions. No setup required."""
    pass


@given("I can phrase commands in different ways")
def step_phrase_commands(context):
    """NARRATIVE: User can phrase commands in different ways. No setup required."""
    pass


@given("I need to work with multiple environments")
def step_multiple_environments(context):
    """NARRATIVE: User needs to work with multiple environments. No setup required."""
    pass


@given("I know a VM by its alias")
def step_know_alias(context):
    """NARRATIVE: User knows VMs by their aliases. No setup required."""
    pass


@given("I want to know what's running")
def step_want_status(context):
    """NARRATIVE: User wants to know what VMs are running. No setup required."""
    pass


@given("I'm not sure what to do")
def step_need_help(context):
    """NARRATIVE: User is not sure what commands are available. No setup required."""
    pass


@given("I need to connect to a VM")
def step_need_connect(context):
    """NARRATIVE: User needs to connect to a VM. No setup required."""
    pass


@given("I need to rebuild a container")
def step_need_rebuild(context):
    """NARRATIVE: User needs to rebuild a container. No setup required."""
    pass


@given("I want to operate on all VMs of a type")
def step_operate_by_type(context):
    """NARRATIVE: User wants to operate on all VMs of a specific type. No setup required."""
    pass


@given("I'm done working")
def step_done_working(context):
    """NARRATIVE: User is done working and wants to cleanup. No setup required."""
    pass


@given("I use conversational language")
def step_conversational_language(context):
    """NARRATIVE: User uses conversational language. No setup required."""
    pass


@given("I want to set up a backend")
def step_want_backend(context):
    """NARRATIVE: User wants to set up a backend environment. No setup required."""
    pass
...
# ========== ADDITIONAL GIVEN STEPS (Narrative + Setup) ==========


@given("I type commands in various cases")
def step_type_cases(context):
    """NARRATIVE: User types commands in various cases. No setup required."""
    pass


@given("I want to type less")
def step_type_less(context):
    """NARRATIVE: User wants to type less. No setup required."""
    pass


@given("something isn't working")
def step_troubleshooting(context):
    """Setup: User is troubleshooting an issue - creates postgres for restart test."""
