...
def step_dependent_services(context):
    ...
    assert postgres_exists or redis_exists, (
        "At least one dependent service (postgres or redis) must exist"
    )
...
def step_databases_caches_available(context):
    ...
    running = docker_ps()
    available = [
        vm for vm in running if any(db in vm for db in ["postgres", "redis", "mongo", "mysql"])
    ]
...
def step_can_connect_mysql(context):
    ...
    result = run_vde_command(
        'exec mysql "mysqladmin ping -h localhost -u root -pdevpassword"', context=context
    )
