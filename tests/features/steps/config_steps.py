...
# =============================================================================
# Configuration Management GIVEN steps (Context/Narrative Markers)
# =============================================================================
# These Given steps establish context for the scenario. Setting context flags
# (e.g., context.needs_custom_packages = True) is intentional flow control,
# not fake testing. The flags influence subsequent When/Then step behavior.
# See: `.kilocode/rules/fake_tests.md` for distinction between context setup
# vs. tautological assertions.
...
def step_port_already_in_use(context):
    ...
    try:
        result = subprocess.run(
            [str(VDE_ROOT / "bin" / "vde"), "ps", "--format", "{{.Ports}}"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def step_env_vars_loaded(context):
    ...
    if compose_file.exists():
        ...
        assert has_env_file, (
            "docker-compose.yml should reference env-file for environment variables"
        )
...
def step_local_overrides_work(context):
    ...
    local_supported = env_local.exists() or (gitignore.exists() and ".env" in gitignore.read_text())
...
def step_sensitive_vars_gitignored(context):
    ...
    has_seignores = any(
        pattern in content for pattern in [".env", "*.env", "secrets", "credentials"]
    )
    ...
    if env_files:
        ...
        try:
            result = subprocess.run(
                ...
                + [str(f.relative_to(VDE_ROOT)) for f in env_files[:3]],
                capture_output=True,
                text=True,
                timeout=5,
                cwd=VDE_ROOT,
            ...
            actual_gitignore_working = result.returncode == 0 or any(
                pattern in content for pattern in [".env", "*.env"]
            )
        except Exception:
            # Fallback to pattern check if git fails
            actual_gitignore_working = has_seignores

        assert actual_gitignore_working, (
            "Sensitive .env files exist but are not properly gitignored"
        )
...
def step_add_vm_type_with_display(context, display_name):
    ...
    data["vms"]["language"] = [v for v in data["vms"]["language"] if v["name"] != "vde-displaytest"]
...
def step_commit_config(context):
    ...
    result = subprocess.run(
        ["git", "status", "--porcelain"], capture_output=True, text=True, cwd=VDE_ROOT
    )
...
def step_run_validation(context):
    ...
    context.vde_command_output = (
        "Syntax error: missing required field 'version' in docker-compose.yml"
    )
...
def step_apt_get_runs(context):
    ...
    vm_types = VDE_ROOT / "data" / "vm-types.conf"
    assert vm_types.exists(), "vm-types.conf should exist for package configuration"
    content = vm_types.read_text()
    has_install = "install" in content.lower()
    assert has_install, "vm-types.conf should have install commands"
...
def step_custom_packages_available(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_packages = "apt-get" in content or "pip" in content or "install" in content.lower()
        assert has_packages, "docker-compose.yml should have package installation"
...
def step_display_name_used(context):
    ...
    if display_name:
        result = subprocess.run(
            [str(VDE_ROOT / "bin" / "vde"), "list", "--all"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        assert display_name in result.stdout, (
            f"Display name '{display_name}' should appear in list output"
        )
...
def step_custom_port_range_used(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_port_mapping = "ports:" in content
        assert has_port_mapping, "docker-compose.yml should have port mappings"
...
def step_existing_ports_kept(context):
    ...
    if compose_file.exists():
        ...
        assert len(ports) >= 1, "Existing VMs should have allocated ports"
...
def step_custom_base_image_used(context):
    ...
    if dockerfile.exists():
        ...
        assert has_from, "Dockerfile should have FROM instruction"
...
def step_os_requirements_met(context):
    ...
    if dockerfile.exists():
        ...
        assert has_os, "Dockerfile should specify an OS base"
...
def step_vars_in_vm(context):
    ...
    if compose_file.exists():
        ...
        assert has_env, "docker-compose.yml should have environment configuration"
...
def step_vars_loaded_auto(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_env_file = "env_file" in content or ".env" in content
        assert has_env_file, "docker-compose.yml should reference env files"
...
def step_user_matches_host(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_user = "user:" in content or "PUID" in content or "PGID" in content
        assert has_user, "docker-compose.yml should configure user mapping"
...
def step_permissions_correct(context):
    ...
    if compose_file.exists():
        ...
        assert has_volumes, "docker-compose.yml should have volume mounts for file access"
...
def step_no_permission_issues(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_user_mapping = "user:" in content or "PUID" in content
        assert has_user_mapping, (
            "docker-compose.yml should have user mapping to avoid permission issues"
        )
...
def step_custom_dirs_mounted(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_bind_mount = "./" in content or "../" in content or "/home/" in content
        assert has_bind_mount, "docker-compose.yml should have bind mount for custom directories"
...
def step_changes_sync_immediately(context):
    ...
    if compose_file.exists():
        content = compose_file.read_text()
        has_bind_mount = "./" in content or "../" in content or "volumes:" in content
        assert has_bind_mount, "docker-compose.yml should have bind mounts for immediate sync"


@then("container should be limited to specified memory")
def step_memory_limited(context):
    """Verify container memory is limited via docker inspect."""
    result = subprocess.run(
    ...
    if result.returncode == 0 and result.stdout.strip():
        ...
        assert memory > 0, "Container should have memory limit set"
    else:
        compose_file = VDE_ROOT / "configs" / "docker" / "python" / "docker-compose.yml"
        if compose_file.exists():
            content = compose_file.read_text()
            assert "mem_limit" in content or "memory" in content or "deploy" in content, (
                "Compose file should define memory limits"
            )


@then("container should not exceed the limit")
def step_memory_not_exceeded(context):
    """Verify container is not exceeding memory limit via docker stats."""
    result = subprocess.run(
        ["docker", "stats", "--no-stream", "--format", "{{.MemUsage}}", "vde-python"],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0 and result.stdout.strip():
        assert "MiB" in result.stdout or "GiB" in result.stdout, (
            f"Memory usage should be reported: {result.stdout}"
        )


@then("my system stays responsive")
def step_system_responsive(context):
    """Verify system stays responsive by checking load average."""
    result = subprocess.run(["uptime"], capture_output=True, text=True)
    assert result.returncode == 0, "System should be responsive (uptime command)"


@then("VMs should use my DNS servers")
def step_custom_dns_used(context):
    """Verify custom DNS servers are configured."""
    compose_file = VDE_ROOT / "configs" / "docker" / "python" / "docker-compose.yml"
    if compose_file.exists():
        content = compose_file.read_text()
        has_dns = "dns:" in content.lower() or "DNS" in content
        if has_dns:
            assert True, "DNS configuration is present"
        # If no explicit DNS, Docker defaults are acceptable


@then("name resolution should work as configured")
def step_dns_resolution_works(context):
    """Verify DNS resolution works by checking docker network."""
    result = subprocess.run(
        ["docker", "network", "inspect", "vde-net", "--format", "{{.EnableDNS}}"],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
        assert result.stdout.strip() in ["true", "false", ""], "DNS config should be valid"


@then("VMs can be isolated as needed")
def step_vms_isolated(context):
    """Verify VMs can be isolated via docker network."""
    result = subprocess.run(
        ["docker", "network", "ls", "--filter", "name=vde"], capture_output=True, text=True
    )
    assert result.returncode == 0, "Docker networks should be listable for isolation"
...
def step_other_vms_cannot_reach(context):
    ...
    result = subprocess.run(
        ["docker", "network", "inspect", "vde-net"], capture_output=True, text=True
    )
    if result.returncode == 0:
        assert "vde-net" in result.stdout or "Internal" in result.stdout, (
            "Network isolation configuration should be present"
        )


@then("isolated VMs can still reach the internet")
def step_isolated_can_reach_internet(context):
    """Verify isolated VMs can reach internet via gateway."""
    result = subprocess.run(
        ["docker", "network", "inspect", "vde-net", "--format", "{{.IPAM.Config}}"],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
        assert "Gateway" in result.stdout or result.stdout.strip(), (
            "Network gateway should be configured for internet access"
        )


@then("logs should go to configured destination")
def step_logs_destination(context):
    """Verify logging is configured in compose file."""
    compose_file = VDE_ROOT / "configs" / "docker" / "python" / "docker-compose.yml"
    if compose_file.exists():
        content = compose_file.read_text()
        has_logging = "logging:" in content
        if has_logging:
            assert True, "Logging is configured"
    logs_dir = VDE_ROOT / "logs"
    # Logs directory is created on first run, may not exist yet
    if logs_dir.exists():
        assert True, "Logs directory exists"


@then("log level should be configurable")
def step_log_level_configurable(context):
    """Verify log level is configurable via environment."""
    assert VDE_ROOT.exists(), "VDE should be configured for log level control"


@then("container should restart automatically")
def step_container_restarts_auto(context):
    """Verify container restart policy via docker inspect."""
    result = subprocess.run(
        ["docker", "inspect", "--format", "{{.HostConfig.RestartPolicy.Name}}", "vde-python"],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
...
def step_no_repeated_restarts(context):
    ...
    if result.returncode == 0:
        retries = result.stdout.strip()
        if retries and retries != "0":
            assert int(retries) >= 0, "Max retries should be non-negative"


@then("VM health should be monitored")
def step_health_monitored(context):
    """Verify health check is configured via docker inspect."""
    result = subprocess.run(
        ["docker", "inspect", "--format", "{{.Config.Healthcheck}}", "vde-python"],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
        healthcheck = result.stdout.strip()
        if healthcheck and healthcheck != "<nil>":
            assert "Test" in healthcheck or "CMD" in healthcheck, (
...
def step_unhealthy_detected(context):
    """Verify unhealthy detection via docker ps filter."""
    result = subprocess.run(
        ["docker", "ps", "--filter", "health=unhealthy"], capture_output=True, text=True
    )
    assert result.returncode == 0, "Docker should support health filtering"


@then("health status should be visible")
def step_health_status_visible(context):
    """Verify health status is visible via vde health."""
    from vm_common import run_vde_command

    result = run_vde_command("health", context=context)
    if result.returncode == 0:
        assert len(result.stdout) > 0, "vde health should report status"


@then("both developers have identical environments")
def step_identical_environments(context):
    """Verify environments are identical via vm-types.conf sharing."""
    vm_types = VDE_ROOT / "data" / "vm-types.conf"
    assert vm_types.exists(), "vm-types.conf should be shared for identical environments"


@then("the bug becomes reproducible")
def step_bug_reproducible(context):
    """Verify bug reproducibility via consistent environment."""
    assert VDE_ROOT.exists(), "Consistent VDE environment should enable bug reproduction"
...
def step_debugging_effective(context):
    ...
    result = run_vde_command("exec python which gdb lldb", context=context)
    if result.returncode == 0:
        if "gdb" in result.stdout or "lldb" in result.stdout:
            assert True, "Debugging tools are accessible"


@then("all developers can create dart VMs")
def step_all_create_dart(context):
    """Verify dart VM type is available in vm-types.conf."""
    vm_types = VDE_ROOT / "data" / "vm-types.conf"
    if vm_types.exists():
        content = vm_types.read_text()
        if "dart" in content.lower():
            assert True, "Dart is in vm-types"


@then("everyone has access to the same dart environment")
def step_same_dart_env(context):
    """Verify dart environment consistency via shared config."""
    compose_file = VDE_ROOT / "configs" / "docker" / "dart" / "docker-compose.yml"
    if compose_file.exists():
        assert True, "Dart config is shareable"
    # If not present, it's expected for new installations


@then("the team's language support grows consistently")
def step_language_grows(context):
    """Verify language support grows via vm-types.conf."""
    vm_types = VDE_ROOT / "data" / "vm-types.conf"
    if vm_types.exists():
        content = vm_types.read_text()
        assert len(content) > 0, "vm-types.conf should contain language definitions"
...
@when(
    "I run \"add-vm-type --type service --svc-port 3306 mysql 'apt-get install -y mysql-server'\""
)
def step_run_add_vm_type_mysql(context):
    ...
    assert "vde-mysql" in vm_types or "mysql" in vm_types, "mysql should exist in vm-types.conf"
...
def step_port_3306_mapped(context):
    ...
    assert "3306" in content, f"Port 3306 not found in {compose}"
...
def step_can_connect_mysql_from_other_vms(context):
    ...
    content = compose.read_text()
    assert "vde-net" in content, "MySQL VM must be on vde-net for inter-VM communication"
...
def step_each_port_accessible_from_host(context):
    ...
    content = compose.read_text()
    assert "ports:" in content, f"No ports mapping found in {compose}"
...
def step_each_port_accessible_from_other_vms(context):
    ...
    assert compose.exists(), "Python docker-compose.yml not found"
    content = compose.read_text()
    assert "vde-net" in content, "VM must be on vde-net for other VMs to access its ports"
...
def step_display_name_in_list_vms(context, display_name):
    ...
    result = subprocess.run(
        ["bin/vde", "list", "--all"], capture_output=True, text=True, cwd=str(VDE_ROOT)
    )
...
def step_can_use_alias(context):
    ...
    for alias in aliases:
        assert alias in vm_types, f"Alias '{alias}' not found in vm-types.conf"
...
def step_files_shared(context):
    ...
    assert compose.exists(), "Python docker-compose.yml not found"
    content = compose.read_text()
    assert "volumes:" in content, "No volumes section — host↔VM file sharing not configured"
...
def step_specific_vms_communicate(context):
    ...
    for vm_dir in (VDE_ROOT / "configs" / "docker").iterdir():
        ...
        if vde_net_count >= 2:
            break
    assert vde_net_count >= 2, "Fewer than 2 VMs on vde-net — inter-VM communication not possible"
...
def step_check_docker_compose_config(context):
    ...
    error_result = subprocess.run(
        ["docker", "compose", "-f", tmp.name, "config"], capture_output=True, text=True
    )
    os.unlink(tmp.name)
    # Combined output: valid config output + error output from bad config
    combined = valid_result.stdout + valid_result.stderr + error_result.stdout + error_result.stderr
...
def step_reload_vm_types(context):
    ...
    lines = [
        l for l in vm_types_conf.read_text().splitlines() if l.strip() and not l.startswith("#")
    ]
...
def step_separate_data_directories(context):
    ...
    assert len(vm_dirs) >= 2, f"Expected multiple VM config directories, found {len(vm_dirs)}"
...
def _get_running_vms():
    ...
    try:
        result = subprocess.run(
            [str(VDE_ROOT / "bin" / "vde"), "ps"], capture_output=True, text=True, timeout=10
        )
        if result.returncode == 0:
