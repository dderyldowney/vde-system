...
class RemediationPlanGenerator:
    ...
    def generate_plan(self, output_file: Path):
        ...
        sorted_categories = sorted(
            grouped.items(), key=lambda x: (severity_order.get(x[1][0]["severity"], 4), -len(x[1]))
        )
        ...
        with open(output_file, "w") as f:
            ...
            for severity_level in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
                fixes_in_level = [
                    (cat, fails)
                    for cat, fails in sorted_categories
    ...
    def _write_fix_section(self, f, fix_num: int, category: str, failures: List) -> int:
        ...
        for failure in failures[:5]:  # Show first 5
            if "feature_file" in failure:
                f.write(
                    f"- {failure['feature_file']}:{failure.get('scenario_line', '?')} - {failure['scenario']}\n"
                )
        ...
        fixes = {
            "DEBUG_OUTPUT": {
                ...
                "fix": [
                    ...
                    "Update parser script that loads `data/vm-types.conf`",
            ...
            "SHELL_COMPAT": {
                ...
                "files": [
                    "tests/features/docker-free/shell-compatibility.feature",
                    "shell compat scripts",
                ],
            },
            "SSH_AGENT": {
