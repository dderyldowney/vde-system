...
# Test no API key
unset CLAUDE_API_KEY ANTHROPIC_API_KEY
...
# Test ANTHROPIC_API_KEY is preferred
ANTHROPIC_API_KEY="sk-ant-test123"
CLAUDE_API_KEY="sk-claude-test456"
KEY=$(_get_ai_api_key)
assert_equals "sk-ant-test123" "$KEY" "ANTHROPIC_API_KEY should be preferred"
...
ANTHROPIC_API_KEY="sk-ant-test123"
if ai_api_available >/dev/null 2>&1; then
    echo -e "${GREEN}✓${NC} correctly reports available with API key"
else
    echo -e "${RED}✗${NC} should be available with API key"
fi
((TESTS_PASSED++))
((TESTS_RUN++))
...
# Test Zhipu AI configuration scenario
ANTHROPIC_BASE_URL="https://api.z.ai/api/anthropic"
ANTHROPIC_DEFAULT_SONNET_MODEL="glm-4.7"
ANTHROPIC_API_KEY="zhipu-test-key"
...
assert_equals "zhipu-test-key" "$KEY" "Zhipu: API key"

# Reset
unset ANTHROPIC_BASE_URL ANTHROPIC_DEFAULT_SONNET_MODEL ANTHROPIC_API_KEY

# Test corporate proxy scenario
ANTHROPIC_BASE_URL="https://api-gateway.internal.company.com/anthropic/v1"
ANTHROPIC_MODEL="claude-3-5-sonnet-20241022"
ANTHROPIC_API_KEY="corp-test-key"
...
assert_equals "corp-test-key" "$KEY" "Corporate: API key"

# Reset
unset ANTHROPIC_BASE_URL ANTHROPIC_MODEL ANTHROPIC_API_KEY
