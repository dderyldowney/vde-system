...
| `vde enter <vm>` | The Sovereign Handshake: Enter a Spoke's login shell. | `vde enter rust` |
| `vde rebuild <vm>` | Re-forge a Spoke's Docker image (defaults to no-cache). | `vde rebuild python` |
| `vde remove <vm>` | Dissolve (remove) a VM instance and its locks. | `vde remove rust` |
...
| `vde ps` | List all running VDE-managed containers. | `vde ps` |
