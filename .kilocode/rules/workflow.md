...
Entry: Code changes complete (Phase 2) | Action: Run `yume-guardian` on all changes | Loop: If fails, use `yume-implementer` to fix and re-run until CLEAN | Constraint: No git actions allowed during this phase | Exit Gate: Yume-guardian returns CLEAN (zero violations) | Violation Protocol: Git actions during audit → STOP immediately
## Phase 4: Code Review
