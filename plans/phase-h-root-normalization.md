*** Begin Patch
*** Add File: plans/phase-h-root-normalization.md
+# Phase H: Root Normalization & Phase H Actions
+
+Goals
+- Enforce VDE_ROOT_DIR as the sole root for all host-side operations.
+- Prevent hard-coded absolute paths; ensure all code uses "$VDE_ROOT_DIR/…".
+- Introduce runtime guards and basic observability hooks for phase verification.
+
+Plan & Actions
...
+  - Ensure lib/vde-root-guard.zsh is sourced early in startup to detect leaks at runtime.
