...
local vde_rabbitmq_pkgs="erlang-nox rabbitmq-server"
...
if [[ "true" == "true" ]]; then
    ...
    touch "${_zshenv}"
    grep -q "service rabbitmq-server start" "${_zshenv}" || echo "sudo service rabbitmq-server start >/dev/null 2>&1" >> "${_zshenv}"
